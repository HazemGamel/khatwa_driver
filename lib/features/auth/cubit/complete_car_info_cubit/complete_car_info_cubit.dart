...
import '../../presentation/views/upload_driver_data_view/criminal_view.dart';
...
class CompleteCarInfoCubit extends Cubit<CompleteCarInfoState> {
  ...
  void pickDriverCriminalImage(bool isCamera) async {
    emit(PickDriverCriminalImageLoadingState());
    driverCriminal = await _pickImage.pickImage(isCamera);
    emit(PickDriverCriminalImageState());
  }
...
/////////////////////
  ...
  uploadDriverCarLicenseImage() async {
    ...
    result.fold((l) {
      ...
      navigateTo(UploadCriminalView());
    });
  }

  //upload criminal
  uploadCriminalImage() async {
    emit(CriminalLoadingState());
    var r = await step();
    final result = await _carInfoRepo.uploadImage(
        image: driverCriminal!,
        title: 'record',
        uri: Uri.parse('${baseUri}request/$r/criminal-record'));
    result.fold((l) {
      emit(CriminalErrorState(l));
    }, (r) {
      emit(CriminalSuccessState());
